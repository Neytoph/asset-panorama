...
数据主权（本地文件，无云端无账号）、agent 即引导和编辑器（没有向导 UI，对你的 agent
说一句话就能配好）、储蓄率是唯一完全可控的变量（现金流驱动定投）、再平衡是纪律不是预测
（IPS 宪法 + 5/25 容忍带）、工具是仪式的主持人不是人生的记分员（年度复盘 + 人生目标问答）。

每一条背后都有一次真实决策或真实翻车，完整版见 **[docs/why.md](docs/why.md)** ——
如果你只读这个仓库的一份文档，读它;你的 agent 也应该读它。
